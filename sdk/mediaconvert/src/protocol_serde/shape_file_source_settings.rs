...
pub fn ser_file_source_settings(
    ...
    if let Some(var_6) = &input.time_delta {
        object.key("timeDelta").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
                        ...
                        match key.to_unescaped()?.as_ref() {
                            ...
                            "convertPaintToPop" => {
                                builder = builder.set_convert_paint_to_pop(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::CaptionSourceConvertPaintOnToPopOn::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
