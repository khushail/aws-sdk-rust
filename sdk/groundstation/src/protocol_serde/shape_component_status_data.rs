...
pub fn ser_component_status_data(
    ...
    if let Some(var_4) = &input.bytes_sent {
        object.key("bytesSent").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_4).into()));
    }
    if let Some(var_5) = &input.bytes_received {
        object.key("bytesReceived").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_5).into()));
    }
    if let Some(var_6) = &input.packets_dropped {
        object.key("packetsDropped").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_6).into()));
    }
