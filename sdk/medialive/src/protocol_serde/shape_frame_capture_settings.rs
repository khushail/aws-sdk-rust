...
pub fn ser_frame_capture_settings(
    ...
    if let Some(var_1) = &input.capture_interval {
        object.key("captureInterval").number(#[allow(clippy::useless_conversion)]::aws_smithy_types::Number::NegInt((*var_1).into()));
    }
    ...
    if let Some(var_3) = &input.timecode_burnin_settings {
        ...
        crate::protocol_serde::shape_timecode_burnin_settings::ser_timecode_burnin_settings(&mut object_4, var_3)?;
        object_4.finish();
    }
    Ok(())
}
                                ...
                                builder = builder.set_capture_interval_units(
                                    ::aws_smithy_json::deserialize::token::expect_string_or_null(tokens.next())?.map(|s|
                                        s.to_unescaped().map(|u|
                                            crate::types::FrameCaptureIntervalUnit::from(u.as_ref())
                                        )
                                    ).transpose()?
                                );
                            }
