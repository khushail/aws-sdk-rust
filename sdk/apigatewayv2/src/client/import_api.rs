...
impl super::Client  {
    ...
    ///   - [`api_gateway_managed(Option<bool>)`](crate::operation::import_api::ImportApiOutput::api_gateway_managed): <p>Specifies whether an API is managed by API Gateway. You can't update or delete a managed API by using API Gateway. A managed API can be deleted only through the tooling or service that created it.</p>
