// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
impl super::Client  {
    /// Constructs a fluent builder for the [`DescribeEntitlements`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder) operation.
                            ///
                            /// - The fluent builder is configurable:
    ///   - [`name(impl ::std::convert::Into<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::name) / [`set_name(Option<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::set_name): <p>The name of the entitlement.</p>
    ///   - [`stack_name(impl ::std::convert::Into<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::stack_name) / [`set_stack_name(Option<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::set_stack_name): <p>The name of the stack with which the entitlement is associated.</p>
    ///   - [`next_token(impl ::std::convert::Into<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::next_token) / [`set_next_token(Option<String>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::set_next_token): <p>The pagination token used to retrieve the next page of results for this operation.</p>
    ///   - [`max_results(i32)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::max_results) / [`set_max_results(Option<i32>)`](crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::set_max_results): <p>The maximum size of each page of results.</p>
                            /// - On success, responds with [`DescribeEntitlementsOutput`](crate::operation::describe_entitlements::DescribeEntitlementsOutput) with field(s):
    ///   - [`entitlements(Option<Vec<Entitlement>>)`](crate::operation::describe_entitlements::DescribeEntitlementsOutput::entitlements): <p>The entitlements.</p>
    ///   - [`next_token(Option<String>)`](crate::operation::describe_entitlements::DescribeEntitlementsOutput::next_token): <p>The pagination token used to retrieve the next page of results for this operation.</p>
                            /// - On failure, responds with [`SdkError<DescribeEntitlementsError>`](crate::operation::describe_entitlements::DescribeEntitlementsError)
    pub fn describe_entitlements(&self) -> crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder {
                                crate::operation::describe_entitlements::builders::DescribeEntitlementsFluentBuilder::new(self.handle.clone())
                            }
}

