...
pub fn de_public_key_list(decoder: &mut ::aws_smithy_xml::decode::ScopedDecoder) -> Result<crate::types::PublicKeyList, ::aws_smithy_xml::decode::XmlDecodeError> {
    #[allow(unused_mut)]
    let mut builder = crate::types::PublicKeyList::builder();
    while let Some(mut tag) = decoder.next_tag() {
        match tag.start_el() {
            ...
            s if s.matches("Items") /* Items com.amazonaws.cloudfront#PublicKeyList$Items */ =>  {
                let var_4 =
                    Some(
                        crate::protocol_serde::shape_public_key_summary_list::de_public_key_summary_list(&mut tag)
                        ?
                    )
                ;
