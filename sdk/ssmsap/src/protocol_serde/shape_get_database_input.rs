// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
pub fn ser_get_database_input(
                     object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
                     input: &crate::operation::get_database::GetDatabaseInput,
                ) -> Result<(), ::aws_smithy_http::operation::error::SerializationError> {
    if let Some(var_1) = &input.application_id {
        object.key("ApplicationId").string(var_1.as_str());
    }
    if let Some(var_2) = &input.component_id {
        object.key("ComponentId").string(var_2.as_str());
    }
    if let Some(var_3) = &input.database_arn {
        object.key("DatabaseArn").string(var_3.as_str());
    }
    if let Some(var_4) = &input.database_id {
        object.key("DatabaseId").string(var_4.as_str());
    }
    Ok(())
}

